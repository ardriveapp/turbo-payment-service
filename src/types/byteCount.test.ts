...
describe("ByteCount class", () => {
  describe("constructor", () => {
    it("constructs valid ByteCounts given healthy inputs", () => {
      ...
      byteCountInputs.forEach((byteCount) =>
        expect(() => new ByteCount(byteCount)).to.not.throw(Error)
      );
    });

    it("throws an error when provided invalid inputs", () => {
      const byteCountInputs = [
        -1,
        0.5,
      ...
      byteCountInputs.forEach((byteCount) =>
        expect(
          () => new ByteCount(byteCount),
          `${byteCount} should throw`
        ).to.throw(Error)
      );
    });
  });
  ...
  describe("roundToChunkSize function", () => {
    it("returns the correct ByteCount number value", () => {
      const eid = new ByteCount(12345);
      const rounded = new ByteCount(262144);
  ...
  describe("isGreaterThanOrEqualTo function", () => {
    it("returns false when other ByteCount is greater", () => {
      expect(new ByteCount(1).isGreaterThanOrEqualTo(new ByteCount(2))).to.be
        .false;
    });

    it("returns true when other ByteCount is lesser", () => {
      expect(new ByteCount(2).isGreaterThanOrEqualTo(new ByteCount(1))).to.be
        .true;
    });

    it("returns true when other ByteCount is equal", () => {
      expect(new ByteCount(2).isGreaterThanOrEqualTo(new ByteCount(2))).to.be
        .true;
    });
  });
});
