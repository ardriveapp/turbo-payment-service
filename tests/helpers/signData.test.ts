...
describe("signData", () => {
  it("snapshot test for signing data with JWK", async () => {
    const signature = await signData(testWallet, "123");
  ...
  describe("signedRequestHeadersFromJwk", () => {
    it("should create correct headers for signed data", async () => {
      ...
      const signedData = await signData(testWallet, data + nonce);
      ...
      expect(signedHeaders["x-public-key"]).to.equal(testWallet.n);
